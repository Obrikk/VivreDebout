...
function Sorties() {
    ...
    const tileContent = ({ date, view }) => {
        if (view === 'month') {
            const sortie = sorties.find(sortie => {
                const sortieDate = new Date(sortie.date);
                return date.getDate() === sortieDate.getDate() &&
                       date.getMonth() === sortieDate.getMonth() &&
                       date.getFullYear() === sortieDate.getFullYear();
            });
            if (sortie) {
                return (
            ...
            <div className='sorties'>
                ...
                <Flex w='100%' justify={'center'}>
                    <Calendar
                        ...
                        value={dateState}
                        tileContent={tileContent} // Utilise la fonction tileContent pour déterminer le contenu des cases du calendrier
                        tileClassName={tileClassName} // Utilise la fonction tileClassName pour ajouter la classe aux cases du calendrier
                    ...
                    />
                    <Flex  w='25rem' padding={'1rem'}>
                        <Text display={'Flex'} alignItems={'center'}><div id="circle"></div>OUI</Text>
                    </Flex>
                ...
                <Grid gridTemplateColumns='repeat(3, 1fr)' width='80%' mt='1rem' gap='4rem' fontFamily='Helvetica'>
                    {sortiesSortedByDate && sortiesSortedByDate.length > 0 ? sortiesSortedByDate.map((sortie) => (
                        <Flex key={sortie.id} bg='#d6efff' width={{lg:'100%'}} borderRadius='20px' boxShadow='rgba(100, 100, 111, 0.2) 0px 7px 29px 0px;' height={{lg:'17rem'}}
                         direction='column' justify='space-between' alignItems='center' cursor='pointer' as={motion.div} whileHover={{ scale: 1.03 }}
                          initial={{ x: '-20rem' }} padding='1rem' animate={{ x: '0rem' }} whileTap={{ scale: 0.99 }}>
