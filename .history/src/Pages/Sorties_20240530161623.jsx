            ...
            <div className={'sorties'}>
                ...
                <Grid gridTemplateColumns={{ xl: 'repeat(3,1fr)' }} width={{ xl: '80%' }} marginTop={{ xl: '5rem' }} gap={{ xl: '4rem' }} fontFamily={'Helvetica'}>
                    {sorties.length > 0 ? sorties.map((sortie) => (
                        <Flex key={sortie.id} bg='#' borderRadius={'20px'} boxShadow={'rgba(100, 100, 111, 0.2) 0px 7px 29px 0px;'} height={{ xl: '24rem' }} direction='column' justify={'center'} alignItems='center' cursor='pointer' as={motion.div} whileHover={{ scale: 1.1 }} initial={{ x: '-20rem' }} animate={{ x: '0rem' }} whileTap={{ scale: 0.9 }}>
