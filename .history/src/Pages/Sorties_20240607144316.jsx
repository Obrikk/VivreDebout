...
import {
    ...
    DrawerContent, DrawerCloseButton, Image
...
function Sorties() {
    ...
    const tileContent = ({ date, view }) => {
        if (view === 'month' && date.getDate() === 8 && date.getMonth() === 5) {
            // Retourne un tableau d'éléments React avec le contenu et la classe
            return [
                <span key="content">Oui</span>,
                <div key="className" className="selectedDate" />
            ];
        }
        return null; // Retourne null pour les autres jours
    };
    ...
    useEffect(() => {
        ...
        document.querySelector('.button-45').style.backgroundColor = '#B4E1FF'
        return () => {
    ...
    const handleDateChange = (date) => {
        ...
        useEffect(() => {
          ...
          setToken(storedToken); // Set token state with retrieved value from localStorage*
...
function checkTokenExpire(token) {
    ...
    try {
        ...
        if (decodedToken.exp < currentTime) {
            ...
            console.log('Token expiré, supprimé du localStorage');
            return true;
        } else {
            // Token valide
        ...
        // Une erreur s'est produite lors du décodage du token
        console.error('Erreur lors du décodage du token :', error);
        return true; // Suppose que le token est invalide en cas d'erreur
    }
}
useEffect(() => {
    ...
    if (storedToken) {
        const isTokenExpired = checkTokenExpire(storedToken);

        // Si le token est expiré, supprimer-le du localStorage
        if (isTokenExpired) {
            localStorage.removeItem('token');
        }
    }
    ...
    return (
        <>
            ...
            <div className='sorties'>
                ...
                value={dateState}
                tileContent={tileContent} // Utilise la fonction tileContent pour déterminer le contenu et la classe des cases du calendrier
                tileClassName={({ date, view }) =>
                    view === 'month' && date.getDate() === 8 && date.getMonth() === 5
                        ? 'selectedDate' // Ajoute la classe 'selectedDate' uniquement pour le 8 juin
                        : null
                }
                />
                ...
                <Grid gridTemplateColumns='repeat(3, 1fr)' width='80%' mt='1rem' gap='4rem' fontFamily='Helvetica'>
                    {sortiesSortedByDate && sortiesSortedByDate.length > 0 ? sortiesSortedByDate.map((sortie) => (
                        <Flex key={sortie.id} bg='#d6efff' width={{lg:'100%'}} borderRadius='20px' boxShadow='rgba(100, 100, 111, 0.2) 0px 7px 29px 0px;' height={{lg:'17rem'}}
                         direction='column' justify='space-between' alignItems='center' cursor='pointer' as={motion.div} whileHover={{ scale: 1.1 }}
                          initial={{ x: '-20rem' }} padding='1rem' animate={{ x: '0rem' }} whileTap={{ scale: 0.9 }}>
                            <Text fontSize='20px' fontWeight='600'>{sortie.nom}</Text>
                            {token && jwtDecode(token).role === 'admin' &&(
                                <Image src={Croix} onClick={() => handleDelete(sortie.id)} alt="Suprrimer sortie bouton"  w={{lg:'1.5rem'}} pos={'relative'}  bottom={{md:'2.5rem'}} left={'7rem'}/>

                            )

                            }
                            <Text>Type d'activité:  {sortie.type}</Text>
                            <Text>{formatDate(sortie.date)}</Text>
                            <Text>{sortie.horaires}</Text>
                            <Text>{sortie.lieu}</Text>
                            <Button bg='white'>Description</Button>
                        </Flex>
                    )) : (
                        <Text key={'no-sorties'}>il n'y a pas de sorties prévues</Text>
                    )}
                </Grid>
                    ...
                    <DrawerContent>
                        ...
                        <DrawerBody>
                            <form id='my-form' onSubmit={handleSubmit}>
                                ...
                                <DatePicker className='date-picker' selected={selectedDate} onChange={handleDateChange} dateFormat="yyyy-MM-dd" />
