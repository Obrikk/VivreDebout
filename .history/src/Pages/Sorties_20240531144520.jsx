    ...
    return (
        <>
            ...
            <div className='sorties'>
                <Heading size='3xl' mt='2rem'>Sorties</Heading>
                <Grid gridTemplateColumns='repeat(3, 1fr)' width='80%' mt='5rem' gap='4rem' fontFamily='Helvetica'>
                    {sorties && sorties.length > 0 ? sorties.map((sortie) => (
                        <Flex key={sortie.id} bg='#d6efff' borderRadius='20px' boxShadow='rgba(100, 100, 111, 0.2) 0px 7px 29px 0px;' height='24rem' direction='column' justify='center' alignItems='center' cursor='pointer' as={motion.div} whileHover={{ scale: 1.1 }} initial={{ x: '-20rem' }} animate={{ x: '0rem' }} whileTap={{ scale: 0.9 }}>
            ...
            <Footer>
                <Button onClick={onOpen} position='absolute' top='10rem' left='10rem'>Ajouter</Button>
            </Footer>
        </>
    );
}
